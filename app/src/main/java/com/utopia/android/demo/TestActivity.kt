...
class TestActivity: AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.testButton.setOnClickListener {
                    ...
                    private fun flush(message: String?) {
                        ...
                        try {
                            ...
                            if (lastPosition > 0) {
                                ...
                                trace("mCacheMapOperator isAppendSuccess=")
                                if (!isAppendSuccess) {
                            ...
                            if (msgByteArraySize > 0) {
                                ...
                                trace("mLogFileMapOperator isAppendSuccess=")
                                if (!isAppendSuccess) {
            ...
            """.trimIndent()
            executor.execute {
                ULog.toLogcat("content_tag", "content size=${content.length}")
                for (index in 0 until 1000000) {
                    ULog.d(TAG, "index=${index} ========${content}=========")
                }
            }
        }
